...
// impl PartialEq<AssetInfo> for AssetInfo {
//     fn eq(&self, rhs: &AssetInfo) -> bool {
//         match self {
...
impl AssetInfo {
    ...
    pub fn query_balance(&self, querier: &QuerierWrapper, pool_addr: Addr) -> StdResult<Uint128> {
        match self {
    ...
    pub fn query_symbol(&self, querier: &QuerierWrapper) -> StdResult<String> {
        match self {
            AssetInfo::Token { contract_addr, .. } => {
                query_token_symbol(querier, contract_addr.clone())
            }
            AssetInfo::NativeToken { denom, .. } => {
                Ok(denom.clone())
            }
